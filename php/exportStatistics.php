...
if(isset($_SESSION['signedIn'])){
    ...
    $q = $db->query("SELECT Coop_company, count(*) FROM student_info group by Coop_company ORDER BY count(*) DESC LIMIT 6;");
    $counter = 0;
    while($row = $q->fetch()){
    ...
    $q = $db->query("SELECT Company_size, count(*) FROM student_info group by Company_size;");
    $counter = 0;
    while($row = $q->fetch()){
    ...
    $q = $db->query("SELECT Major, count(*) FROM student_info group by Major ORDER BY Major;");
    $counter = 0;
    while($row = $q->fetch()){
        // Temp solution guys
    ...
    $q = $db->query("SELECT Nationality, count(*) FROM student_info WHERE Nationality='Saudia Arabia';");
    $counter = 0;
        ...
        $forthChart[$counter]['Value'] = $db->query("SELECT Nationality, count(*) FROM student_info WHERE Nationality!='Saudia Arabia';")->fetch()[1];
